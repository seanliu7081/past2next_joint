...
class LazyZarrDataset(BaseDataset):
    """
    Like ZarrDataset but opens the zarr lazily from disk instead of
    copying it fully into RAM.  Each __getitem__ reads only the
    requested slice, keeping memory usage constant regardless of
    dataset size.
    """
    def __init__(
        self,
        ...
        sampler_keys = [action_key] + list(obs_keys)
