...
| obs feature dim (`cond_dim`) | Fused vision(128)+state(10) per obs step | **138** | `FusedObservationEncoder.output_feature_dim()` |
...
- **Eval (`model.eval()`):** uses the explicit `eval_keep_k` list if provided, else no dropout (full 8 latents). At inference (`detokenize`), `eval_keep_k=token_lens` is passed (a list of per-sample token lengths) so the decoder keeps exactly the number of generated tokens per sample.
...
[fused_obs_encoder.py](oat/perception/fused_obs_encoder.py). Fuses **vision** + **state** per observation step; text disabled for libero10. `cond_dim = output_feature_dim() = 138`.

### 6.1 Vision (`RobomimicRgbEncoder`, crop `76×76`)

[robomimic_vision_encoder.py](oat/perception/robomimic_vision_encoder.py). Two RGB ports: `agentview_rgb`, `robot0_eye_in_hand_rgb`, each `[B, To=2, 128, 128, 3]`. `share_rgb_model` defaults to `False`, so each port owns an independent `visual_net`.

- **CropRandomizer:** train = random `76×76` crop; eval = center `76×76` crop.
- **Backbone:** `ResNet18Conv`, BatchNorm replaced with **GroupNorm** (`groups = features//16`) for small-batch stability.
- **Pooling:** `SpatialSoftmax`, `num_kp=32`, temperature 1.0 → 32 keypoints × 2 coords, flattened.
- **Per-`visual_net` output feature dim:** **64** (`VisualCore(feature_dimension=64)`, i.e. the flattened SpatialSoftmax output of one port). With 2 ports the robomimic `ObservationEncoder.output_shape()` **sums** them, so the encoder's combined output is **128**. `output_feature_dim()` computes `D = output_shape()[0] = 128`, allocates `D//N = 64` per port across the `N=2` ports, and returns their `sum = 128` (per-port allocation 64; combined total 128).
...
Per step, modality features are **concatenated** along the feature axis: `cat([vision(128), state(10)], -1) = 138`. Output `[B, To=2, 138]`. `set_normalizer` fans out to both sub-encoders; missing per-port params log a warning and skip (graceful).
...
AutoregressiveModel(
    ...
    cond_dim = 138,              # obs feature dim
    n_layer = 4, n_head = 4, n_emb = 256,
    p_drop_emb = 0.1, p_drop_attn = 0.1,
)
```

**Embeddings.** Token: `tok_emb = Embedding(1001, 256)`, weight-**tied** to the output `head`. Absolute learned positions `tok_pos_emb (1, 9, 256)` (zero-init). Condition: `cond_emb = Linear(138→256)`, `cond_pos_emb (1, 2, 256)`.
...
- **AdamW betas:** `[0.9, 0.95]`. **weight_decay:** `0.0` everywhere in the shipped config (the decay/no-decay split only matters if `weight_decay > 0`: weights/embeddings `dim≥2` would decay, biases/norms `dim<2` would not).
...
`predict_action` is **inherited from `OATPolicy`** ([oatpolicy.py](oat/policy/oatpolicy.py) lines 170–216) — the joint policy overrides `__init__`, `set_normalizer`, `get_optimizer`, `forward`, and `get_policy_name` (which swaps the policy-name prefix from `oatpolicy_` to `oat_joint_`). `set_ce_weight` is a **new** method added by the joint policy (not an override of a base-class method). `predict_action` and `detokenize` are reused unchanged.

```
obs_dict ─► obs_encoder ─► features [B, 2, 138]
          start = [<BOS>=1000]  [B,1]
...
- **`detokenize`** ([tokenizer.py](oat/tokenizer/oat/tokenizer.py) lines 121–143): pad to `latent_horizon` if short, `FSQ.indices_to_embedding` → latents `[B,8,4]`, then `decode(latents, eval_keep_k=token_lens)` where `token_lens` is a list of per-sample token lengths (constructed as `[tokens.shape[1]] * tokens.shape[0]` at line 134, e.g. `[8, 8, …, 8]` when every sample generated 8 tokens). The decoder runs in eval mode with that per-sample `eval_keep_k` list → `unnormalize` → `[B,32,7]`.
...
| [oat/perception/fused_obs_encoder.py](oat/perception/fused_obs_encoder.py) | `FusedObservationEncoder` — vision+state fusion (dim 138) |
